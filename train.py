...
def create_alphafold_module(
    ...
    torch_xpu_rng_state = torch.xpu.get_rng_state(device=device)
    np.random.seed(seed % NUMPY_SEED_MODULUS)
    torch.manual_seed(seed)
    alphafold = AlphaFold(config=alphafold_config)
    alphafold.to(device=device)
    print("UT: alphafold mapped to device ",device,"\n")
    torch.xpu.set_rng_state(torch_xpu_rng_state, device=device)
...
def training(args: argparse.Namespace) -> None:
    if args.distributed:
        if "MPI_RUN" in os.environ:
            ...
            torch.distributed.init_process_group(backend="ccl", init_method="env://", rank=rank, world_size=world_size)
        else:
    ...
    # Distributed wrapper:
    if args.distributed:
        alphafold = torch.nn.parallel.DistributedDataParallel(module=alphafold, device_ids=[device], output_device=device)
    ...
    for iteration in range(1, args.num_train_iters + 1):
        ...
        if (
            ...
            if is_main_process:
                print("... in save_checkpoint_from_training")
